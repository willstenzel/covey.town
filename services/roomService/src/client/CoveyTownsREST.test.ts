...
describe('TownsServiceAPIREST', () => {
  ...
  describe('CoveyJoinQueueAPI', () => {
    it('Can sucessfully join a queue', async () => {
      const pubTown1 = await createTownForTesting(undefined, true);
      const res = await apiClient.joinTown({
        userName: nanoid(),
        coveyTownID: pubTown1.coveyTownID,
      });
      const joinQueueRes = await apiClient.joinQueue({
        playerID: res.coveyUserID,
        coveyTownID: pubTown1.coveyTownID,
      });
      expect(joinQueueRes.queuePosition).toEqual(1);
    });
    it('Throws and error if the playerID is invalid', async () => {
      const pubTown1 = await createTownForTesting(undefined, true);
      await apiClient.joinTown({
        userName: nanoid(),
        coveyTownID: pubTown1.coveyTownID,
      });
      const joinQueueRequest = async () =>
        apiClient.joinQueue({
          playerID: nanoid(),
          coveyTownID: pubTown1.coveyTownID,
        });
      await expect(joinQueueRequest).rejects.toThrow();
    });
    it('Throws an error if the coveyTownID is invalid', async () => {
      const pubTown1 = await createTownForTesting(undefined, true);
      const res = await apiClient.joinTown({
        userName: nanoid(),
        coveyTownID: pubTown1.coveyTownID,
      ...
      await expect(joinQueueRequest).rejects.toThrow(
        'Error processing request: Error: No such town',
      );
    });
    it('Throws and error if the player is already in the queue', async () => {
      const pubTown1 = await createTownForTesting(undefined, true);
      const res = await apiClient.joinTown({
        userName: nanoid(),
        coveyTownID: pubTown1.coveyTownID,
      ...
      await joinQueueRequest();
      await expect(joinQueueRequest).rejects.toThrow();
    });
  });

  describe('CoveyHelpNextAPI', () => {
    it('Can sucessfully help the next student in the queue', async () => {
      const pubTown1 = await createTownForTesting(undefined, true);
      const res = await apiClient.joinTown({
        userName: nanoid(),
        coveyTownID: pubTown1.coveyTownID,
      ...
      const helpNextStudentRes = await apiClient.helpNextStudent({
        coveyTownID: pubTown1.coveyTownID,
      });
      expect(helpNextStudentRes).toEqual({ helped: true });
    });
    it('Throws an error if the coveyTownID is invalid', async () => {
      const pubTown1 = await createTownForTesting(undefined, true);
      const res = await apiClient.joinTown({
      ...
      const helpNextStudentRes = () =>
        apiClient.helpNextStudent({
          coveyTownID: nanoid(),
        });
      await expect(helpNextStudentRes).rejects.toThrow(
        'Error processing request: Error: No such town',
      );
    });
    it('Throws an error if the queue is empty', async () => {
      ...
      await expect(helpNextStudentRes).rejects.toThrow(
        'Error processing request: Error: Unable to help next student',
      );
    });
  });
});
